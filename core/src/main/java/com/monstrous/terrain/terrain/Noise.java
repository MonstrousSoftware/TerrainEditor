...
public class Noise {
    ...
    public float[][] smoothNoise (int width, int height, float[][] baseNoise) {
        ...
        float[][] smoothedNoise = new float[width][height]; // an array of floats initialised to 0
        ...
        for (int octave = 0; octave < octaves; octave++) {
            ...
            for (int i = 0; i < width; i++) {
                for (int j = 0; j < height; j++) {
                    smoothedNoise[i][j] += smoothNoise[i][j] * amplitude;
                }
            }
        }
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                smoothedNoise[i][j] /= totalAmplitude;
            }
        }
